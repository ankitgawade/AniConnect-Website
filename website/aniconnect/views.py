...
def receive_ratings(request):
	if request.method == 'POST':
		user=request.user
		user_ratings = Review.objects.filter(user=user).values_list('anime_id', 'rating')
		
		anime_ids, ratings = zip(*user_ratings)
		data = {'anime_ids': anime_ids, 'ratings': ratings}
        # Send JSON data to Google Colab notebook
		colab_url = 'https://417a-34-145-93-165.ngrok-free.app/returndata'
		response = requests.post(colab_url, json=data)
		print()
		if response.status_code == 200:
			modified_data = response.json()
			return home_with_data(request, modified_data)
		else:
			return JsonResponse({'error': 'Failed to trigger notebook'})
	else:
...
def home_with_data(request, modified_data):
	user = request.user
	existing_recommendations = RecommendedAnime.objects.filter(user=user)
	existing_recommendations.delete()
	for x in modified_data:
		anime = AniconnectAniDesc.objects.get(pk=x)
        # Create a UserAnime object and associate it with the current user and anime
		user_anime = RecommendedAnime(user=user, anime=anime)
		user_anime.save()
	recommended_animes=RecommendedAnime.objects.filter(user=user)
	return render(request, 'home.html', {'recommended_animes': recommended_animes})
def search(request):
	if request.method == 'POST':
